...
def align_time_indices(portfolio_returns: pd.Series, benchmark_data: pd.DataFrame) -> Tuple[pd.Series, pd.Series]:
    ...
    aligned_benchmark_returns = benchmark_data.loc[common_dates, 'total_return']
...
def validate_time_ranges(asset_data: List[pd.DataFrame]) -> Tuple[pd.Timestamp, pd.Timestamp]:
    current_date = pd.to_datetime('2025-03-19')  # Current date
    min_dates = []
    max_dates = []
    ...
    for df in asset_data:
        ...
        df_filtered = df[df['time'] <= current_date]
        if df_filtered.empty:
            continue
        min_dates.append(df_filtered['time'].min())
        max_dates.append(df_filtered['time'].max())
    
    if not min_dates or not max_dates:
        raise ValueError("No valid time ranges found in asset data after filtering future dates.")
    
    start_date = max(min_dates)  # Latest start date
    end_date = min(max_dates)    # Earliest end date
    
    if start_date > end_date:
        raise ValueError("Assets do not have overlapping time ranges after filtering future dates.")
    
    # Cap end_date at current date
    if end_date > current_date:
        st.warning(f"End date {end_date} is in the future. Capping at current date: {current_date}.")
...
def plot_growth_of_assets(asset_data: List[pd.DataFrame], asset_names: List[str], portfolio_returns: pd.Series, benchmark_data: pd.DataFrame) -> None:
    ...
    if not benchmark_data.empty:
        if 'time' in benchmark_data.columns and 'return' in benchmark_data.columns:
            ...
            benchmark_tmp['Growth'] = (1 + benchmark_data['total_return']).cumprod()
...
def fetch_benchmark_data(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    try:
        ...
        mapped_data['total_return'] = mapped_data['price'].pct_change()
        mapped_data['price_return'] = mapped_data['total_return']  # No dividends from yfinance, so total_return = price_return
        mapped_data['dividend_yield'] = 0.0
        mapped_data['dividend'] = 0.0
        mapped_data.dropna(subset=['total_return'], inplace=True)
        return mapped_data[['time', 'total_return', 'price_return', 'dividend_yield', 'price', 'dividend']]
    except Exception as e:
        raise ValueError(f"Error processing benchmark data: {e}")

def adjust_column_names(df: pd.DataFrame) -> pd.DataFrame:
    expected_columns = ['time', 'total_return', 'price_return', 'dividend_yield', 'price', 'dividend']
...
def validate_columns(df: pd.DataFrame) -> None:
    # At least 'time' and one of 'total_return', 'price_return', or 'price' must be present
    if 'time' not in df.columns:
        raise ValueError("Missing required column: 'time'")
    if not any(col in df.columns for col in ['total_return', 'price_return', 'price']):
        raise ValueError("At least one of 'total_return', 'price_return', or 'price' must be present")
 
def calculate_missing_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Attempt to ensure df has usable columns:
      - total_return
      - price_return
      - dividend_yield
      - price
    ...
    We handle the following main scenarios:

    1) total_return is already present (and not all NaN) => do nothing special.
    2) If total_return is missing, but price_return is present => we can compute total_return as price_return + dividend_yield.
       If dividend_yield is missing but dividend + price exist, we compute dividend_yield = dividend / price.shift(1).
    3) If price_return is also missing, but price is present => we compute price_return from price.pct_change().
    4) If still missing, we raise an error because there's no way to get total_return.

    That covers the typical “8 cases” logic:
      - (Yes/No) total_return
      - (Yes/No) price_return
      - (Yes/No) dividend_yield
      - (Yes/No) price
      - (Yes/No) dividend
    """
    df = df.copy()
    
    # Standardize required columns (create them if missing, filled with NaN)
    for col in ["time", "total_return", "price_return", "dividend_yield", "price", "dividend"]:
        if col not in df.columns:
            df[col] = np.nan

    # Convert to numeric where applicable
    numeric_cols = ["total_return", "price_return", "dividend_yield", "price", "dividend"]
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    # Ensure 'time' is datetime and sort by time
    df["time"] = pd.to_datetime(df["time"])
    df = df.sort_values("time")

    # --- 1) If we have a valid total_return (not all NaN), we leave it as-is ---
    has_valid_total_return = not df["total_return"].isna().all()

    # --- 2) If total_return is missing or all NaN, try to compute it ---
    if not has_valid_total_return:
        # (a) If price is given, we can compute price_return if missing
        if df["price_return"].isna().all() and not df["price"].isna().all():
            df["price_return"] = df["price"].pct_change()

        # (b) If dividend_yield is missing but we have price + dividend, compute dividend_yield
        if df["dividend_yield"].isna().all() and not df["price"].isna().all() and not df["dividend"].isna().all():
            df["dividend_yield"] = df["dividend"] / df["price"].shift(1)

        # (c) If we now have price_return, we can compute total_return = price_return + dividend_yield
        #     If dividend_yield is still missing, treat it as zero
        if not df["price_return"].isna().all():
            df["total_return"] = df["price_return"] + df["dividend_yield"].fillna(0)

    # --- 3) Final check: if total_return is still all NaN, we must fail ---
    if df["total_return"].isna().all():
        raise ValueError(
            "Cannot proceed: 'total_return' is missing or all NaN, and none of the other columns "
            "('price_return', 'price', 'dividend_yield', 'dividend') were sufficient to compute it. "
            "Please check your data."
        )

    # Drop rows where total_return ended up NaN (e.g. first row with a pct_change, etc.)
    initial_len = len(df)
    df = df.dropna(subset=["total_return"])
    dropped = initial_len - len(df)
    

    # Final check for sufficiency
    if df.empty:
        raise ValueError("No valid rows remain after computing 'total_return'; all data was invalid or dropped.")
...
def calculate_metrics(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, float], Dict[str, float]]:
    if df.empty:
        raise ValueError("Empty DataFrame provided.")
    
    # Calculate missing columns (total_return, price_return, dividend_yield, etc.)
...
def align_asset_data(asset_data: List[pd.DataFrame], asset_names: List[str]) -> pd.DataFrame:
    named = []
    for name, df in zip(asset_names, asset_data):
        if df.empty:
            st.warning(f"No data for {name}, skipping.")
            continue

        # ensure 'time' is a datetime index, then drop the column
        df = df.copy()
        df['time'] = pd.to_datetime(df['time'])
        df = df.set_index('time', drop=True)

        # pick off only the return series, renamed to the asset name
        named.append(df['total_return'].rename(name))

    if not named:
        st.error("No asset data available for alignment.")
        return pd.DataFrame()

    # INNER join — keeps only dates present in *all* assets
    aligned = pd.concat(named, axis=1, join='inner')
    aligned.index.name = 'time'     # explicitly name the index
    return aligned
    ...
    if asset_benchmarks and all(not b.empty for b in asset_benchmarks):
        ...
        for i, bench_df in enumerate(asset_benchmarks):
            if not bench_df.empty:
                bench_returns = bench_df.set_index('time')['total_return'].reindex(aligned_returns.index).fillna(0)
        ...
        weighted_benchmark_returns = benchmark_data.set_index('time')['total_return'].reindex(aligned_returns.index).fillna(0)
    else:
    ...
    for asset, returns_series in asset_returns_dict.items():
        ...
        for period in periods:
            ...
            if period == "ITD":
                ...
                sliced_returns = returns_series[start_date:end_date]
                sliced_bench = bench_returns[start_date:end_date]
                sliced_risk_free = risk_free_series[start_date:end_date]
            else:
                ...
                start_date = max(start_date, returns_series.index.min())
                sliced_returns = returns_series[start_date:end_date]
                sliced_bench = bench_returns[start_date:end_date]
                sliced_risk_free = risk_free_series[start_date:end_date].fillna(0.0)
...
def display_full_periodic_table(fund_returns: pd.Series, bench_returns: pd.Series, risk_free_series: pd.Series) -> None:
    ...
    five_year = slice_and_calc("5Y")
    itd = slice_and_calc("ITD")
...
def plot_dividend_decomposition(df: pd.DataFrame, asset_name: str) -> None:
    ...
    # Calculate missing columns to ensure 'price_return' and 'dividend_yield' are present
    df = calculate_missing_columns(df)
    
    # Use 'dividend_yield' as the dividend return component
    df['dividend_return'] = df['dividend_yield']
    ...
    # Compute the percentage of total return attributable to dividends for each period (only for positive total_return)
    df['dividend_pct'] = np.where(df['total_return'] > 0, 
                                  (df['dividend_yield'] / df['total_return']) * 100, 
                                  np.nan)
...
def main():
    ...
    with st.sidebar:
        ...
        num_assets = st.number_input(
            "**Number of Assets**", min_value=1, max_value=100, value=1, help="Select the number of assets in your portfolio."
        )
        ...
        for i in range(num_assets):
            with st.expander(f"Asset {i+1}"):
                ...
                # Process benchmark data after asset data is available (restored original logic)
                if benchmark_type == "Ticker":
                    ...
                    if bench_ticker and not asset_data[-1].empty:
                        try:
                            ...
                            # Use calculate_missing_columns to align with new format
                            bench_df = calculate_missing_columns(bench_df)
                            asset_benchmarks.append(bench_df)
                        except Exception as e:
                            st.error(f"Error fetching benchmark data for {asset_name}: {str(e)}")
                    ...
                    if bench_file:
                        try:
                            ...
                            if 'total_return' not in bench_df.columns or bench_df['total_return'].isna().all():
                                if 'price' in bench_df.columns and not bench_df['price'].isna().all():
                                    bench_df['total_return'] = bench_df['price'].pct_change()
                                elif 'return' in bench_df.columns and not bench_df['return'].isna().all():
                                    bench_df['total_return'] = bench_df['return']
                                    bench_df = bench_df.drop(columns=['return'])
                            # Use calculate_missing_columns to align with new format
                            bench_df = calculate_missing_columns(bench_df)
                            asset_benchmarks.append(bench_df)
                        except Exception as e:
                            st.error(f"Error processing benchmark CSV for {asset_name}: {str(e)}")
                            asset_benchmarks.append(pd.DataFrame())
                    else:
                        asset_benchmarks.append(pd.DataFrame())
                else:
    ...
    if benchmark_source == "Fetch from Ticker":
        ...
        if benchmark_file is not None:
            try:
                ...
                if ('price' not in benchmark_data.columns or benchmark_data['price'].isnull().all()) and ('return' in benchmark_data.columns and not benchmark_data['return'].isnull().all()):
                    benchmark_data['price'] = (1 + benchmark_data['return']).cumprod()
                elif ('price' in benchmark_data.columns and not benchmark_data['price'].isnull().all()) and ('return' not in benchmark_data.columns or benchmark_data['return'].isnull().all()):
                    benchmark_data['return'] = benchmark_data['price'].pct_change()
                else:
                    raise ValueError("Either 'price' or 'return' must be provided with valid data in the benchmark CSV.")
        ...
        try:
            ...
            if not use_global_benchmark:
                for name, bench_df in zip(asset_names, asset_benchmarks):
                    asset_benchmarks_dict[name] = bench_df.set_index('time')['total_return'].reindex(portfolio_rets.index, method='ffill') if not bench_df.empty else pd.Series(0.0, index=portfolio_rets.index)
            else:
            ...
            with tab1:  # Portfolio Analysis
                ...
                with st.expander("Portfolio Performance & Rebalancing Simulation", expanded=False):
                    ...
                    if len(growth_date_range) == 2 and growth_date_range[0] <= growth_date_range[1]:
                        ...
                        if len(asset_data) > 1:
                            growth_df =  (1 + filtered_rets) \
                                            .cumprod()               \
                                            .rename("Growth")        \
                                            .reset_index()           \
                            
                            growth_df["Label"] = "Portfolio"
                            
                            if not filtered_bench.empty:
                                bench_df = (1 + filtered_bench).cumprod().rename("Growth") \
                                        .reset_index()
                                bench_df["Label"] = "Benchmark"
